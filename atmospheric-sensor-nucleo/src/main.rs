...
fn main() -> ! {
    ...
    let mut i2c_scl = gpiob.pb8.into_af4_open_drain(&mut gpiob.moder, &mut gpiob.otyper, &mut gpiob.afrh);
    let mut i2c_sda = gpiob.pb9.into_af4_open_drain(&mut gpiob.moder, &mut gpiob.otyper, &mut gpiob.afrh);
    i2c_scl.internal_pull_up(&mut gpiob.pupdr, true);
    i2c_sda.internal_pull_up(&mut gpiob.pupdr, true);
    let i2c = hal::i2c::I2c::new(board.I2C1, (i2c_scl, i2c_sda), 100.kHz().try_into().unwrap(), clocks, &mut rcc.apb1);

    let mut gpioa = board.GPIOA.split(&mut rcc.ahb);
    let serial_tx = gpioa.pa2.into_af7_push_pull(&mut gpioa.moder, &mut gpioa.otyper, &mut gpioa.afrl);
    let mut serial_rx = gpioa.pa3.into_af7_push_pull(&mut gpioa.moder, &mut gpioa.otyper, &mut gpioa.afrl);
    serial_rx.internal_pull_up(&mut gpioa.pupdr, true);
    let mut serial = hal::serial::Serial::new(board.USART2, (serial_tx, serial_rx), 115200.Bd(), clocks, &mut rcc.apb1);
    ...
    loop {
        ...
        if let Ok(Some(measurement)) = sensor.read() {
            let msg = ReportCO2Data { measurement: measurement.co2 };
